...
export class BackendConstruct extends Construct {
  ...
  private createAgentCoreGateway(config: AppConfig): void {
    // ========================================
    // Gobler tool Lambdas (one Lambda per tool — replaces the OOTB sample_tool).
    // Each folder under gateway/tools/<tool>/ contains index.py (handler=index.handler)
    // and tool_spec.json. To add another tool, add an entry here and a Cedar action.
    // ========================================
    // NOTE: Bedrock ConverseStream enforces a 64-char limit on the composed tool
    // name "{mcpPrefix}_{targetName}___{toolName}" (prefix "gateway_" here).
    // Keep target names SHORT so the composed name stays <= 64 chars, or the model
    // call fails with a ValidationException and the agent returns no response.
    const goblerTools = [
      { id: "LookThroughExposure", dir: "look_through_exposure", target: "exposure" },
      { id: "TaxRebalancing", dir: "tax_rebalancing", target: "rebalance" },
      { id: "OpportunisticDeployment", dir: "opportunistic_deployment", target: "deploy" },
    ]

    const toolLambdas: Record<string, lambda.Function> = {}
    for (const tool of goblerTools) {
      toolLambdas[tool.dir] = new lambda.Function(this, `${tool.id}Lambda`, {
        runtime: lambda.Runtime.PYTHON_3_13,
        handler: "index.handler",
        code: lambda.Code.fromAsset(path.join(__dirname, `../../gateway/tools/${tool.dir}`)), // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
        timeout: cdk.Duration.seconds(30),
        logGroup: new logs.LogGroup(this, `${tool.id}LambdaLogGroup`, {
          logGroupName: `/aws/lambda/${config.stack_name_base}-${tool.dir}`,
          retention: logs.RetentionDays.ONE_WEEK,
          removalPolicy: cdk.RemovalPolicy.DESTROY,
        }),
      })
    ...
    // Lambda invoke permission for every Gobler tool Lambda
    for (const fn of Object.values(toolLambdas)) {
    ...
    // Register each Gobler tool Lambda as its own L2 Gateway Target.
    // addLambdaTarget() grants the gateway role invoke permission and adds the
    // resource-based Lambda permission the CreateGatewayTarget dry-run requires.
    // The agent discovers all targets dynamically over MCP (see patterns/.../tools/gateway.py),
    // so no frontend change is needed for tools to appear.
    const gatewayTargets = goblerTools.map(tool =>
      gateway.addLambdaTarget(`${tool.id}Target`, {
        gatewayTargetName: tool.target,
        description: `Gobler ${tool.dir} Lambda target`,
        lambdaFunction: toolLambdas[tool.dir],
        toolSchema: agentcore.ToolSchema.fromLocalAsset(
          path.join(__dirname, `../../gateway/tools/${tool.dir}/tool_spec.json`) // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
        ),
    ...
    // Policy must be created after the Gateway and all its targets are ready
    for (const t of gatewayTargets) {
      cedarPolicy.node.addDependency(t)
    }
    ...
    new cdk.CfnOutput(this, "GatewayTargetIds", {
      value: gatewayTargets.map(t => t.targetId).join(","),
      description: "AgentCore Gateway Target IDs (Gobler tools)",
    })

    new cdk.CfnOutput(this, "ToolLambdaArns", {
      description: "ARNs of the Gobler tool Lambdas",
      value: Object.values(toolLambdas).map(fn => fn.functionArn).join(","),
    })
